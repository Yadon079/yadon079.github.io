...
컴파일러는 지네릭 타입을 이용해서 소스파일을 체크하고, 필요한 곳에 형변환을 넣어준다. 그리고 지네릭 타입을 제거한다. 따라서 컴파일된 파일(*.class)에는 지네릭 타입에 대한 정보가 없다.

지네릭 타입의 기본적인 제거과정은 다음과 같다.  

1. 지네릭 타입의 경계(bound)를 제거한다.  
지네릭 타입이 <T extends Fruit>라면 T는 Fruit로 치환된다. <T>인 경우 Object로 치환된다. 그리고 클래스 옆의 선언은 제거된다.
...
열거형은 서로 관련된 상수를 편리하게 선언하기 위한 것으로 여러 상수를 정의할 때 유용하다.
